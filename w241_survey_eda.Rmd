...
```{r}
# Hisograms of Reasonable Score
histogram(~ total_all | levels(treat), data = df, 
...
```{r}
model6 = lm(reasonable ~ treat + total_all + baseline_spend + factor(meal_ordered), data = df)
summary(model6)

model6b = lm(reasonable ~ treat + total_all + baseline_spend + meal_duck + meal_fettuccine + meal_fish + meal_pizza + meal_pork, data = df)
summary(model6b)
...
```{r}
model8 = lm(reasonable ~ treat + baseline_spend + total_all, data = df)
summary(model8)
```
...
```{r}
# EVERYTHING
model9 = lm(reasonable ~ treat + factor(age) + factor(gender) + baseline_tip_pct + baseline_eat_out + baseline_spend + total_all + factor(meal_ordered) + factor(drink_ordered) + Q2_colleague + Q2_spouse + Q2_alone + Q2_friends + Q2_family + Q3_dessert + Q3_appetizer + Q3_mainCourse + Q4_wine + Q4_cocktail + Q4_beer + Q4_non_alc + Q4_water, data = df)
summary(model9)
```
